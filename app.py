...
def load_model():
    if not os.path.exists(MODEL_PATH):
        ...
        if output is None or not os.path.exists(MODEL_PATH):
            raise RuntimeError(
                "Model download failed. Check Google Drive sharing: Anyone with the link → Viewer."
            )
...
def safe_open_image(uploaded_file):
    try:
        ...
        return None, "This file is not a valid image. Please upload JPG, JPEG, PNG, WEBP, or BMP."
    except Exception as e:
        return None, f"Image could not be processed: {e}"
...
def get_green_ratio(image: Image.Image) -> float:
    ...
    green_mask = (g > r * 1.03) & (g > b * 1.03) & (g > 35)
...
with st.sidebar:
    ...
    st.write("AI dashboard for tomato leaf disease classification.")
    st.divider()
    st.markdown("### Model Info")
    st.write("Input size: **128 × 128**")
    st.write("Classes: **10 tomato categories**")
    st.write("Supported images: **JPG, JPEG, PNG, WEBP, BMP**")
    
    st.divider()
    st.markdown("### Accuracy Note")
    st.write(
        "The app can be stable, but final disease accuracy depends on your trained model and image quality."
    )
    st.write(
        "For true non-tomato detection, add a training class named **Not_Tomato_Leaf**."
    )
...
st.markdown(
    ...
    <div class="hero">
        ...
        <p>Upload or capture a leaf image. The dashboard predicts disease, shows exact confidence %, and gives practical recommendations.</p>
    </div>
    """,
    unsafe_allow_html=True
)
...
st.markdown(
    ...
    <span class="small-badge">🖼 Multi-format Upload</span>
...
if uploaded_image is not None:
    ...
    with col2:
        ...
        if green_ratio < GREEN_RATIO_THRESHOLD:
            ...
            st.write(
                "Please upload a clear tomato leaf photo with the leaf covering most of the image. "
                "The current image may be unclear, background-heavy, too dark, or not a leaf."
            )
            ...
            st.write(
                "The image may not be clear enough, may not be a tomato leaf, or the disease pattern is weak."
            )
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tab3:
        ...
        st.write(
            "This app is for academic/project use. It uses your trained model, so prediction accuracy depends "
            "on the dataset quality, training accuracy, image clarity, lighting, and whether the uploaded image "
            "matches the tomato leaf images used during training."
        )
        st.write(
